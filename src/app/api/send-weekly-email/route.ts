...
export async function GET() {
  return NextResponse.json({ 
    status: 'ready',
    message: 'Email service is ready for weekly reports',
    instructions: 'Send POST request with email address to send weekly report',
    schedule: 'Automated reports sent every Monday at 9 AM',
    recipient: process.env.EMAIL_RECIPIENT || 'fangasisye@gmail.com'
  });
}
